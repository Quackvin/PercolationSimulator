...
typedef struct SearchStack sstack_t;

struct Cluster {
    int node_count;
    node_t *nodeHead;
    cluster_t *neighbourHead;
    cluster_t *next;
} ;
...
list_t *dfs(site_t lattice[NROWS][NCOLS], char *perc_type, int perc_dir) ;
...
int main(int argc, char *argv[]) {
    ...
    start = clock();
    list_t *allClusters = dfs(lattice, argv[1], (int) strtol(argv[3], NULL, 10));
    ...
    for (int i = 0; i < NROWS; i++) {
        for (int j = 0; j < NCOLS; j++) {
            if (!lattice[i][j].seen && lattice[i][j].free) {
                clusterCount++;
                cluster_t *head = createCluster(id);
                searchNode(lattice, i, j, &head, perc_type);
...
void searchNode(site_t lattice[NROWS][NCOLS], int row, int col, cluster_t **head, char *perc_type) {
    ...
    while (stack != NULL) {
        ...
        if (!lattice[p_row][p_col].seen) {
            ...
            addNodeToCluster(head, p_row, p_col);
